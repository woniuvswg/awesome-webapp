...
async def create_pool(loop, **kw):
    ...
    __pool = await aiomysql.create_pool(
        host=kw.get('host', default='localhost'),
        port=kw.get('port', default=3306),
        user=kw['user'],
        password=kw['password'],
        db=kw['db'],
        charset=kw.get('charset', default='utf-8'),
        autocommit=kw.get('autocommit', default=True),
...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        attrs['__insert__'] = 'INSERT INTO `%s` (%s, `%s`) VALUES (%s)' % (tableName, ', '.join(escaped_field), primaryKey, create_args_string(len(escaped_field)))
